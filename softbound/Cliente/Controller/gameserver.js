...
module.exports = class GameServer {
    constructor(id, server_options, maxPlayers, websocketserver,multiworld,pending_messages,last_account_info) {
        ...
        this.onAccountEnter(function (account) {
            ...
            if (Date.now() < account.player.gameserverevent && account.player.server_check === self.id) {
                if (1611784800000 === account.player.gameserverevent) {
                    this.evento500 = true;
                } else {
                    this.evento200 = true;
                }
            ...
            if (self.chathistory !== null) {
                ...
                if (self.evento200 === true) {
                    ...
                    data.push(['¡EVENTO! GP & Gold: 200% - '+w+' para finalizar.', '', 17]);
                }
                if (self.evento500 === true) {
                    data.push([' El porcentaje de GP & Gold cambiaron a 500%', '[Inicio de Evento', 6]);
                }
                
                if (self.name === 'Holiday') {
                    data.push([' Búscame, gáname y te llevas un regalo: (gift) '+account.player.gifts_holiday+' Regalos enviados (gift)', 'Saul', 5]);
                    ...
                    //data.push(['(gift) ¡ Ya esta todo listo !, Prepárense. (party)', 'AntiCovi', 5]);
                } else {}
               /*if (self.name === 'Prix')
                ...
                data.push(['Bienvenido: '+account.player.game_id+' :D', '', 9]);
                //data.push(['The server will enter maintenance mode - 5:00 PM (Peruvian Time)', '', 17]);
				//data.push(['Jose] (gift) Ya Disponible en la Tienda Lucky Egg (Double GP) (party)', '', 5]);
            ...
            account.onExit(function () {
                ...
                if (account.player.lucky_egg_seg > 0) {
                    self.db.updateLuckyEggSegByIdAcc(parseInt(account.player.lucky_egg_seg - secondsremaining(account.player.lucky_egg_item)), account.player.user_id);
                    //Logger.magenta("Tiempo de actividad dentro del server en segundos: "+secondsremaining(account.player.lucky_egg_item));
                }
                account.hasEnteredGame = false;
                account.login_complete = false;
                self.removeAccount(account);
    ...
    searchAccountById(user_id){
       for(var server_id in this.multiworld){
         var account = this.multiworld[server_id].getAccountById(user_id);
         if(account!=null)
            return account;
       }
       return null;
