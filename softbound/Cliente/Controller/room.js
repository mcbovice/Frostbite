...
module.exports = class Room {
    ...
    gameStart(account) {
        ...
        if (self.status === Types.ROOM_STATUS.WAITING || self.status === Types.ROOM_STATUS.FULL) {
            if (((self.player_count > 0) && self.game_mode === Types.GAME_MODE.BOSS && self.checkReady()) || ((self.player_count > 1) && self.game_mode === Types.GAME_MODE.NORMAL && self.team_a_count === self.team_b_count && self.checkReady()) || ((self.player_count > 1) && self.game_mode === Types.GAME_MODE.SAME && self.team_a_count === self.team_b_count && self.checkReady()) || ((self.player_count > 1) && self.game_mode === Types.GAME_MODE.SCORE && self.team_a_count === self.team_b_count && self.checkReady()) || ((self.player_count > 1) && self.game_mode === Types.GAME_MODE.TAG && self.team_a_count === self.team_b_count && self.checkReady())) {
                ...
                if (self.gameserver.server_subtype === 3) {
                    var info_prix = ArrayToObject(account.player.tournament, "start_time end_time players avatar_on max_wind force_mobile name total_games last_5_minutes_games rooms maps game_mode s1 tp save_personal save_guild min_points different_mobiles gifts gp_event".split(" "));
                    if (self.gameserver.name === 'Bunge.') {
                        ...
                        var data_maps_server = info_prix.maps;
                        var room_random = data_maps_server [ Math.floor ( Math.random() * data_maps_server.length )];
                        if (room_random === 42 || room_random === 43 || room_random === 44 || room_random === 45)
                            room_random = parseInt(room_random - 2);
                        self.map = Types.MAPS_PLAY[room_random];//19
                    }
                }
                    ...
                    self.game.start(function (fturn) {
                        self.forPlayers(function (account) {
                            if (typeof (account) !== 'undefined') {
                                ...
                                self.win_team_gp = self.team_b_count==1 ? 12 :self.team_b_count==2 ? 24 : self.team_b_count==3 ? 36 : self.team_b_count==4 ? 48 : 12;
                                ...
                                self.win_team_gpb = self.team_bots_count==1 ? 4 :self.team_bots_count==2 ? 4 : self.team_bots_count==3 ? 4 : self.team_bots_count==4 ? 4 : 0;
                        ...
                        self.turn_list.sort(function(a,b){
                            return a.position-b.position;
                        });

                        var lastturn_discount = -8;
                        for(var i=0;i<self.turn_list.length;i++){
                            self.forPlayers(function(account23){
                                    if(account23.player.user_id==self.turn_list[i].user_id)
                                        account23.player.lastturn = lastturn_discount;
                            });
                        ...
                        //Logger.normal("El usuario: "+account.player.game_id+" - inocio el juego en la room: "+self.id+" - Server: "+self.gameserver.id);
                        if (self.gameserver.server_subtype === 3) {
                                        ...
                                        if (typeof (account_loss) !== 'undefined') {
                                            if (team != account_loss.player.team) {
                                                if (typeof (player.no_win_bonus_players[parseInt(account_loss.player.user_id)]) !== 'undefined') {
                                                    if (player.no_win_bonus_players[parseInt(account_loss.player.user_id)].loss >= 1) {
                                                        if (player.no_win_bonus_players[parseInt(account_loss.player.user_id)].loss === 6) {
                                                ...
                                                if (self.gameserver.name === 'Holiday') {
                                                    self.gameserver.forEachAccount(function (accounttp) {
                                                        if (accounttp !== null && accounttp.player.user_id === account.player.user_id) {
                                                            ...
                                                            if (self.gameserver.name === 'Holiday') {
                                                                data_game_server.push([' Búscame, gáname y te llevas un regalo: (gift) '+accounttp.player.gifts_holiday+' Regalos enviados (gift)', 'Saul', 5]);
    ...
    removePlayer(account) {
        ...
        try {
            ...
            } else if (self.player_count === 1 && self.gameserver.server_subtype !== 3 && self.game) {
                self.game.checkDead();
            }
        } catch (e) {
            console.log(e);
