...
module.exports = class Account {
    ...
    Handler(opcode, message) {
        ...
        var self = this;
        var reg = /([^f:]+)/;
        const ip = reg.exec(this.connection._connection._socket.remoteAddress)[0];
        ...
        switch (opcode) {
            ...
            case Types.CLIENT_OPCODE.login:
                {
                    ...
                    .then(function (data) {
                        /*if(data.gm!=1){
                            if(data.rank != 26){
                                self.sendMessage(new Message.alertResponse("Cerrado por mantenimiento", "Esperamos su comprension"));
                            console.log("login incomplete",opcode);self.connection.close();
                            }
                        ...
                        if (data.banned === 1) {
                            ...
                            self.gameserver.checkAccountOnlineAndClose(data.user_id, function () {
                                ...
                                self.player.lucky_egg_item = Date.now();
                                if (self.player.rank >= 0) {
                                    if (self.gameserver.id === 6) {
                                        ...
                                        self.player.tournament = [0,0,2,1,26,-1,"",191588,68,25,[0,1,9,13],0,1,1,0,0,-100000,null,[],100,10,0];
                                        self.sendMessage(new Message.loginResponse(self));
                                    }
                                    else if (self.gameserver.id === 11) {
                                        self.player.tournament = [0,0,0,0,26,-1,"",2552814,1367,489,[0,1,9,13],0,1,1,0,0,-100000,null,[],100,20,0];
                                        self.sendMessage(new Message.loginResponse(self));
                                    } else {}
                                    let next_rank = 0; let check_ranks = JSON.parse(self.player.first_important_ranks);
                                    ...
                                    else if (self.player.gp >= 6900 && self.player.gp <= 7538) { next_rank = 11; }
                                    else if (self.player.gp >= 7539 && self.player.gp <= 8690) { next_rank = 12; }
                                    else if (self.player.gp >= 8691 && self.player.gp <= 10101) { next_rank = 13; }
                                    ...
                                    else if (self.player.gp >= 37347 && self.player.gp <= parseInt(check_ranks[2] - 1)) { next_rank = 21; }
                                    else if (self.player.gp >= check_ranks[2] && self.player.gp <= parseInt(check_ranks[1] - 1)) { next_rank = 22; }
                                    else if (self.player.gp >= check_ranks[1] && self.player.gp <= parseInt(check_ranks[0] - 1)) { next_rank = 23; }
                                    else if (self.player.gp >= check_ranks[0]) { next_rank = 24; } else {}
                                ...
                                self.check_messages();
                                if (1613278800000 >= Date.now()) {
                                    self.send([17,"¡PROMOCIÓN!",'Promoción de cash por tiempo limitado, aprovecha esta gran oferta de cash. <a style="color:#fbf9f9;text-shadow: 0px 0px 2px #ff980099, 0px 0px 3px #ff830057, 0px 0px 7px #ff98005e, 0px 0px 5px #ff9b0066, 0px 0px 8px #ff980059, 0px 0px 8px #ff8f0070;" href="/cash" target="_blank">¡RECARGA YA! - ¡Click Aqui!</a>']);
                                }
            ...
            case Types.CLIENT_OPCODE.send_bcm:
                {
                    ...
                    if (self.player.is_muted === true || self.player.is_muted >= Date.now()) {
                        ...
                        _msj = _msj.replace("//", "");
                        _msj = _msj.replace("%", "");
                        self.gameserver.pushBroadcastChat(new Message.chatResponse(self, _msj, Types.CHAT_TYPE.BUGLE));
                                ...
                                if (valid_precio && !errtrampa) {
                                    ...
                                    if (_iprecio <= 0) {
                                        ...
                                        try {
                                            ...
                                            if (id === 2319) {
                                                self.gameserver.db.updateLuckyEggByUserIdANDaId(item_lucky_egg, self2.player.user_id, id).then(function (dataa) {
                                                    if (dataa.error_mysql || dataa.error_querry) {} else {
                                                        self.gameserver.db.getPlayerAvatars(self2).then(function (dataaa) {
                                                            if (dataaa.error_mysql || dataaa.error_querry) {} else {
                                                                var dat = self.gameserver.avatars.getAvatarDataList(dataaa.data_list);
                                    ...
                                    self.gameserver.db.getBoy(self.player.user_id, id).then(function (acc) {
                                        ...
                                        .then(function (data) {
                                            if (data.error_mysql || data.error_querry) {} else {
                                                ...
                                                if (id === 2319) {
                                                    self.gameserver.db.updateLuckyEggByUserIdANDaId(item_lucky_egg, self.player.user_id, id).then(function (dataa) {
                                                        if (dataa.error_mysql || dataa.error_querry) {} else {
                                                            self.gameserver.db.getPlayerAvatars(self).then(function (dataaa) {
                                                                if (dataaa.error_mysql || dataaa.error_querry) {} else {
                                                                    var dat = self.gameserver.avatars.getAvatarDataList(dataaa.data_list);
                                                                    self.sendMessage(new Message.myAvatars(self, dat));
                                                                }
                    ...
                    if (work) {
                        self.gameserver.db.equipAvatar(arr_up, self)
                            .then(function (data) {
                                if (data.error_mysql || data.error_querry) {} else {
                                    if (self.room) {
                                        self.gameserver.pushToRoom(self.room.id, new Message.roomSlotUpdate(self, self.room), null);
            ...
            case Types.CLIENT_OPCODE.event:
                {
                    ...
                    if (type === 0 && self.player.event1 >= 1) {
                        self.sendMessage(new Message.loginResponse(self)); // segundo cuadro
                        self.sendMessage(new Message.alertResponse('Lo sentimos ', 'usted ya recibió el cash y monto del evento. 😕'));
                        return null;
                    } else if (type === 3 && self.player.event2 >= 1) {
                        self.sendMessage(new Message.loginResponse(self)); // primer cuadro
                        self.sendMessage(new Message.alertResponse('Lo sentimos ', 'usted ya recibió el cash y monto del evento. 😕'));
            ...
            case Types.CLIENT_OPCODE.change_name:
                {
                    ...
                    if (ignoreCase.startsWith(_nname, "GM") || ignoreCase.startsWith(_nname, "DangerBound")) {
                        fuck = true;
                    }
            ...
            case Types.CLIENT_OPCODE.guild_leave:
                {
                    ...
                    if (self.player.guild === "ZeRo.") {
                        self.sendMessage(new Message.alertResponse("Lo sentimos", "Esta opción del Guild esta bloqueada para este gremio. Inténtalo mas tarde."));
                        return null;
                    }
            ...
            case Types.CLIENT_OPCODE.channel_join:
                {
                    ...
                    if (self.room) {
                        if(self.room.watchers[self.user_id]){
                            delete self.room.watchers[self.user_id];
                            self.gameserver.pushToRoom(self.room.id, new Message.watcherLeft(self.user_id), null);
                            self.room = null;
                            self.room_number = 0;
                            self.sendMessage(new Message.loginResponse(self));
                                ...
                                self.gameserver.getRoomById(id, function (room) {
                                    if (room) {
                                        ...
                                        if (self.player.rank <= 16)
                                            rank_bot = 0;
                                        var bot_data = {user_id: id_bot,reg_id: id_bot,game_id: 'NiñaBonita',rank: rank_bot,gp: 0,gold: 0,cash: 0,gender: "m",photo_url: "1234",ahead: 202,abody: 3100,aeyes: 0,aflag: 866,abackground: 0,aforeground: 0,is_muted: 0,guild: 'ℓєgєи♕',guild_id: 0,guild_job: 0};
                                        var plx = new Player(bot_data);
                                        plx.is_bot = 1;
                                        plx.is_ready = 1;
                                        plx.position = 1;
                                        plx.mobile = Types.MOBILE.BOOMER;
                                        plx.scores_lose = 1;
                                        let acc = new Bot(plx);
                                        acc.user_id = id_bot;
                        ...
                        if (room) {
                            if (room.player_count < room.max_players) {
                                ...
                                if (self.gameserver.name === 'Bunge.') {
                                    ...
                                    var room_random = data_maps_server [ Math.floor ( Math.random() * data_maps_server.length )];
                                    if (room_random > 11)
                                        room_random = parseInt(room_random - 1);
                                    if (room_random > 40)
                                        room_random = parseInt(room_random - 2);
                                    room.map = Types.MAPS_PLAY[room_random];
                                }
                                room.joinPlayer(self);
                                self.location_type = Types.LOCATION.ROOM;
                    ...
                    if (self.room) {
                        ...
                        if (self.player.is_master === 1) {
                            ...
                            if (self.room.search_team_room === 1) {//Inicio de codigo del Vs Team
                                ...
                                var start_game = true;
                                self.room.forPlayers(function (account) {
                                    if (self.player.computer_ip === account.player.computer_ip && self.player.team !== self.gameserver.getAccountById(account.player.user_id).player.team) {
                                        start_game = false;
                                    }
                                });
                                if (start_game) {
            ...
            case Types.CLIENT_OPCODE.use_exitem:
                {
                    var use_ex_item = message[1];
                    //Logger.magenta("Use Ex-Item: "+message);
                    if (use_ex_item === "lucky_egg") {
                        if (self.player.lucky_egg_seg <= 0) {
                            self.player.lucky_egg_seg = 3600;
                        } else {
                            self.player.lucky_egg_seg += 3600;
                        }
                        self.gameserver.db.updateLuckyEggSegByIdAcc(self.player.lucky_egg_seg, self.player.user_id).then(function (data) {
                            if (data.error_mysql || data.error_querry) {} else {
                                self.gameserver.db.getUseExItemByUserId(self.player.user_id, 2319).then(function (data2) {
                                    if (data2.error_mysql || data2.error_querry) {} else {
                                        var rows = data2[0][0];
                                        if (rows.use_exitem > 1) {
                                            var ex_item_lucky = rows.use_exitem;
                                            self.gameserver.db.updateLuckyEggByUserIdANDaId(parseInt(ex_item_lucky - 1), self.player.user_id, 2319).then(function (data3) {
                                                if (data3.error_mysql || data3.error_querry) {} else {
                                                    self.gameserver.db.getPlayerAvatars(self).then(function (data4) {
                                                        if (data4.error_mysql || data4.error_querry) {} else {
                                                            var dat = self.gameserver.avatars.getAvatarDataList(data4.data_list);
                                                            self.player.lucky_egg_item = Date.now();
                                                            if (self.room) {
                                                                self.gameserver.pushToRoom(self.room.id, new Message.roomState(self.room));
                                                                self.gameserver.pushToRoom(self.room.id, new Message.roomPlayers(self.room), null);
                                                            }
                                                            self.sendMessage(new Message.myAvatars(self, dat));
                                                            self.sendMessage(new Message.loginResponse(self));
                                            ...
                                            self.gameserver.db.deleteAvatarByUserID(self.player.user_id, 2319).then(function (data) {
                                                if (data.error_mysql || data.error_querry) {} else {
                                                    self.gameserver.db.getPlayerAvatars(self).then(function (data2) {
                                                        if (data2.error_mysql || data2.error_querry) {} else {
                                                            var dat = self.gameserver.avatars.getAvatarDataList(data2.data_list);
                                                            self.player.lucky_egg_item = Date.now();
                                                            if (self.room) {
                                                                self.gameserver.pushToRoom(self.room.id, new Message.roomState(self.room));
                                                                self.gameserver.pushToRoom(self.room.id, new Message.roomPlayers(self.room), null);
            ...
            case Types.CLIENT_OPCODE.game_use_item:
                {
                    ...
                    if (self.room) {
                        ...
                        self.gameserver.pushBroadcastChat(new Message.gameUseItem(self,item_id), self.room);
                        self.send(item_used);
                    }
                    break;
                }
    ...
    Chat(msj, ch) {
        ...
        if (self.player.rank < 3) {
            self.sendMessage(new Message.alertResponse("Hola "+this.player.game_id, "Tu Nivel <span class='span_rank rank rank"+self.player.rank+"'></span> Es Muy Bajo, Nivel mínimo <span class='span_rank rank rank3'></span> es requerido para hablar en este lobby. Juega unas partidas o cambia de server. <img class='emo' src='/static/images/emo/smile.png'>"));
            return null;
        }
        var date = Date.now();
        var cans = false;
        ...
        if (cans === true && self.player.is_muted !== true && self.player.is_muted != 1 || self.player.gm === 1) {
            ...
            if (self.player.rank === 28 && self.player.gm === 1 || self.player.rank === 29 && self.player.gm === 1 || self.player.rank === 30 && self.player.gm === 1)
                type = Types.CHAT_TYPE_BOT;

            if (show) {
                if (ch === true) {
                    if (save === true)
                        self.gameserver.chathistory.push([msj, this.player.game_id, type, this.player.rank, this.player.guild]);
    ...
    if(count > 1){
        ...
        var event_lucky_egg = 0;
        var event_seg_luckyegg = 0;
        if (self.player.power_user === 1)
        ...
        if (self.player.lucky_egg_seg > 0) {
            event_lucky_egg = self.player.lucky_egg_item + (60 * 1000 * self.player.lucky_egg_seg)
            if (event_lucky_egg > Date.now()) {
                event_seg_luckyegg = self.player.win_gp;
            } else {
                self.gameserver.db.deleteAvatarByUserID(self.player.user_id, 2319).then(function (data) {
                    if (data.error_mysql || data.error_querry) {} else {
                        self.gameserver.db.getPlayerAvatars(self).then(function (data2) {
        ...
        self.player.win_gp = parseInt(Math.round(gp_power + plusgp + relation_status + event_seg_luckyegg) + self.player.win_gp);
