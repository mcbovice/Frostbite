...
conn.release();
    ...
    updateLuckyEggFinish(lucky_egg_finish, user_id) {
        var self = this;
        return new Promise(function (resolve, reject) {
            self.connection.getConnection().then(conn => {
                conn.query('UPDATE users SET lucky_egg = ? ,lucky_egg_sec_left = lucky_egg_sec_left-1 WHERE IdAcc = ?', [lucky_egg_finish, user_id])
                    .then(rows => {
                        conn.release();
                        if (rows[0].affectedRows > 0 || rows[0].changedRows > 0)
                            return resolve(rows);
