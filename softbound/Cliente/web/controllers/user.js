    ...
    if (Buffer.byteLength(game_id, 'utf8') < 60) {
        try {
            req.db.getUserByGameId(tmusr)
                ...
                if (rows[0].IdAcc === req.session.account_id || req.session.rank === 31) {
                    my_player = true;
                }
                                    ...
                                    if (rows4[0].length > 0) {
                                        ...
                                        conn.query('SELECT up.post_id, up.user_de, up.user_para, up.texto, up.fecha, u.game_id, u.rank, u.IdAcc FROM user_post up INNER JOIN users u ON u.IdAcc = up.user_de WHERE up.user_para = ? ORDER BY up.post_id DESC limit 0, 19', [rows_x4[0].user_para])
                                            ...
                                            if (rows5[0].length > 0) {
                                                ...
                                                conn.query('SELECT u.IdAcc, u.game_id, u.rank, cp.comment_id, cp.comment_user_para, cp.message_for, cp.time_comment FROM comment_post cp INNER JOIN users u ON cp.comment_user_de = u.IdAcc INNER JOIN user_post up ON up.post_id = cp.comment_user_para WHERE up.user_para = ? limit 0, 19', [rows_x4[0].user_para])
                                                    .then(rows6 => {
                                                    conn.release();
                                                    var answer = rows6[0];
                                                var texto_test = "";
                                                ...
                                                for (i = 0; i < rows_x5.length; i++) {
                                                    ...
                                                    if (rows_x5[i].rank === 26 || rows_x5[i].rank === 27 || rows_x5[i].rank === 31) {
                                                        texto_test += '<div class="boxTitle GMPostTitle">'+"\n";
                                                    } else {
                                                        texto_test += '<div class="boxTitle ">'+"\n";
                                                    }
                                                    texto_test += '<span class="DragonParse PostTitleEmo">(rank'+rows_x5[i].rank+')</span>'+"\n";
                                                    texto_test += '<a class="name" href="/user/'+encodeURIComponent(rows_x5[i].game_id)+'">'+rows_x5[i].game_id+'</a>'+"\n";
                                                    texto_test += '<span class="PostTime">'+"\n";
                                                    texto_test += '<script>'+"\n";
                                                    texto_test += 'TD('+rows_x5[i].fecha+')'+"\n";
                                                    texto_test += '</script>'+"\n";
                                                    texto_test += '</span>'+"\n";
                                                    if (rows[0].IdAcc === req.session.account_id || rows_x5[i].IdAcc === req.session.account_id || req.session.rank === 31)
                                                        texto_test += '<input type="submit" class="DeletePost glow_button" name="action" value="delete">'+"\n";
                                                    texto_test += '</div>'+"\n";
                                                    if (rows_x5[i].rank === 26 || rows_x5[i].rank === 27 || rows_x5[i].rank === 31) {
                                                        texto_test += '<div class="boxBody boxBodyPost DragonParse GMPost">'+rows_x5[i].texto+'</div>'+"\n";
                                                    } else {
                                                        texto_test += '<div class="boxBody boxBodyPost DragonParse">'+rows_x5[i].texto+'</div>'+"\n";
                                                    }
                                                    texto_test += '</form>'+"\n";
                                                    for (var ii = 0; ii < answer.length; ii++) {
                                                        if (answer[ii].comment_user_para === rows_x5[i].post_id) {
                                                            texto_test += '<form action="/post_action" method="POST">'+"\n";
                                                            texto_test += "<input type='hidden' name='csrfmiddlewaretoken' value='5hpTRRAp0ZYRPYIF03ybJNOA5Mjkv5gNqS6I49sbqOM4N4JD7WiQxVXbTbZXtcWz' />"+"\n";
                                                            texto_test += '<input type="hidden" name="CommentID" value="' + answer[ii].comment_id + '">'+"\n";
                                                            texto_test += '<div class="Comment">'+"\n";
                                                            texto_test += '<div class="CommentT ">'+"\n";
                                                            texto_test += '<span class="DragonParse emo">(rank' + answer[ii].rank + ')</span><span class="CommentName"><a class="name" href="/user/' + encodeURIComponent(answer[ii].game_id) + '">' + answer[ii].game_id + '</a>] </span>'+"\n";
                                                            texto_test += '<span class="DragonParse " data-rank="' + answer[ii].rank + '">' + answer[ii].message_for + '</span>'+"\n";
                                                            texto_test += '</div>'+"\n";
                                                            texto_test += '<div class="ShowOnHover">'+"\n";
                                                            if (rows[0].IdAcc === req.session.account_id || rows_x5[i].IdAcc === req.session.account_id || req.session.rank === 31)
                                                                texto_test += '<input type="submit" class="DeleteComment glow_button" name="action" value="deleteComment">'+"\n";
                                                            texto_test += '</div>'+"\n";
                                                            texto_test += '</div>'+"\n";
                                                            texto_test += '</form>'+"\n";
                                                            /*texto_test += '';*/
                                                    ...
                                                    texto_test += '<form action="/post_action" method="POST" class="PostCommentForm">'+"\n";
                                                    texto_test += "<input type='hidden' name='csrfmiddlewaretoken' value='j0I7D1wu7Pr0wEqUh6AAlDjUjhsVgnyjDi9uNSCGY7Hom41UQX4OqypjEWWETdcs' />"+"\n";
                                                    texto_test += '<input type="hidden" name="PostID" value="'+rows_x5[i].post_id+'">'+"\n";
                                                    texto_test += '<input type="hidden" name="action" value="comment">'+"\n";
                                                    texto_test += '<div class="Center">'+"\n";
                                                    texto_test += '<input name="text" class="CommentText" placeholder="Write a comment..." autocomplete="off" maxlength="150">'+"\n";
                                                    texto_test += '</div>'+"\n";
                                                    texto_test += '</form>'+"\n";
                                                    texto_test += '</div>'+"\n";
                                                    texto_test += '<script>DragonParse()</script>'+"\n";
                                                }
                                                res.render('info', {
