...
module.exports = class DataBase {
    ...
    updateLuckyEgg(uses, user_id) {
        var self = this;
        return new Promise(function (resolve, reject) {
            self.connection.getConnection().then(conn => {
                conn.query('UPDATE users SET lucky_egg_sec_left = lucky_egg_sec_left + ? WHERE IdAcc = ?', [uses, user_id])
                    .then(rows => {
                        conn.release();
    ...
    getGuildByName1(guild) {
        var self = this;
        return new Promise(function (resolve, reject) {
            self.connection.getConnection().then(conn => {
                conn.query('SELECT u.game_id, u.gp, u.rank, u.gender, u.IdAcc, g.Id, g.Name, g.points, g.img, g.fondo, g.about, g.website, m.Job FROM users u INNER JOIN guild_member m ON m.UserId = u.IdAcc INNER JOIN guild g ON g.Id = m.Id WHERE g.Name = ? AND m.Job = 1 ORDER BY u.gp DESC', [guild])
                    .then(rows => {
                        conn.release();
                        if (rows[0].length > 0)
                            return resolve(rows);
    ...
    getUserAvatarsByIdAcc(id) {
        ...
        return new Promise(function (resolve, reject) {
            self.connection.getConnection().then(conn => {
                conn.query('SELECT Id, UserId, aId, type, is_cash, is_gift, amount, expire_time, use_exitem FROM user_avatars WHERE UserId = ?  AND remove_ava = 0', [id])
                    .then(rows => {
                        conn.release();
                        if (rows[0].length > 0)
                            return resolve(rows);
    ...
    getSettingsByIdAcc(id) {
        ...
        return new Promise(function (resolve, reject) {
            self.connection.getConnection().then(conn => {
                conn.query('SELECT IdAcc, game_id, gender, photo_url, bg_url, country, rank, gp, win, gender, loss FROM users WHERE IdAcc = ?', [id])
                    .then(rows => {
                        conn.release();
                        if (rows[0].length > 0)
                            return resolve(rows);
    ...
    updateLuckyEggByUserIdANDaId(use_exitem, user_id, ava_id) {
        var self = this;
        return new Promise(function (resolve, reject) {
            self.connection.getConnection().then(conn => {
                conn.query('UPDATE user_avatars SET use_exitem = ? WHERE UserId = ? AND aId = ?', [use_exitem, user_id, ava_id])
                    .then(rows => {
                        conn.release();
                        if (rows[0].affectedRows > 0 || rows[0].changedRows > 0)
                            return resolve(rows);
    ...
    updateLuckyEggSegByIdAcc(lucky_egg, user_id) {
        var self = this;
        return new Promise(function (resolve, reject) {
            self.connection.getConnection().then(conn => {
                conn.query('UPDATE accounts SET lucky_egg_seg = ? WHERE Id = ?', [lucky_egg, user_id])
                    .then(rows => {
                        conn.release();
                        if (rows[0].affectedRows > 0 || rows[0].changedRows > 0)
                            return resolve(rows);
    ...
    getUseExItemByUserId(user_id, ava_id) {
        var self = this;
        return new Promise(function (resolve, reject) {
            self.connection.getConnection().then(conn => {
                conn.query('SELECT use_exitem FROM user_avatars WHERE UserId = ? AND aId = ?', [user_id, ava_id])
                    .then(rows => {
                        conn.release();
                        if (rows[0].length > 0)
                            return resolve(rows);
            ...
            self.getUserByIdAccGame(id)
                .then(function (rows0) {
                    ...
                    if (rows[0].Session === session) {
                        ...
                        data.lucky_egg_seg = rows[0].lucky_egg_seg;
                ...
                .then(function (rows) {
                    if (rows[0].length > 0) {
                        ...
                        for (var xm in res0) {
                            ...
                            lstAvatars.push([res0[xm].Id, res0[xm].aId, equip, res0[xm].type, res0[xm].is_cash, res0[xm].is_gift, res0[xm].expire_time, res0[xm].use_exitem]);
                        }
